...
Every boot validates the door records: doors whose property no longer exists are removed, door entries without a real position are dropped from the property and re-synced, and properties that share a door — the same house created twice — are reported by name and id with their ownership. Such a shared door still opens for anyone who has access to *any* of the properties on that spot, so a buyer is never locked out by a duplicate row, but the duplicate should be deleted from the Manage tab all the same. Two records never get created for one door any more: registration skips a spot that already belongs to another property, creating a property on a door or interior point another property already uses is refused with that property's name, and deleting a property removes its row before anything else, so a failed delete leaves nothing half-done.
...
- **Housing tablet → Housing Management → Keys** is the only place keys come from: cut a spare (`prices.key`) for yourself or hand it straight to someone standing next to you, or **change the lock** (`prices.lock`): every key cut so far dies, you get one fresh key, and an online owner or tenant gets theirs immediately.
- The tablet lives inside the property, so a lost last key means ringing the doorbell and being let in by a roommate before a new one can be cut — hand a spare to someone you trust.
...
- `interactions` (client) — what a wardrobe and the clothing interaction point actually run. `wardrobe` opens the customisation menu and `outfits` the outfit list, each with its own `label` and `icon` for the target option. They ship pointing at illenium-appearance; swap the body of `run` for your own clothing resource's export or event and nothing in the client files needs touching. Add your own entries and point a wardrobe at one with `action = 'yourkey'` in its furniture entry.
